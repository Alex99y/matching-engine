...
// CancelAllOpen cancels every order the account currently has resting in marketRef, plus any
// bracket exit still parked (it holds blocked funds and would otherwise outlive the test).
// Best effort: it is meant for test teardown, so an order that filled or was already cancelled
...
func CancelAllOpen(ctx context.Context, c *client.Client, token, marketRef string) error {
	...
	for _, o := range orders {
		if o.OpenOrder != nil || o.Status == client.StatusPending {
			ids = append(ids, o.ID)
		}
	}
	if len(ids) == 0 {
