...
func (r *InstrumentRepository) CreateNewInstrument(ctx context.Context, name, symbol string, decimals int) error {
	...
	query := `
		...
		VALUES ($1, $2, $3)
	`
	row, err := r.psql.ExecContext(ctxWithTimeout, query, name, symbol, decimals)
	if err != nil {
...
func (r *InstrumentRepository) GetInstrument(ctx context.Context, symbol string) (*Instrument, error) {
	...
	query := `
		...
		WHERE symbol = $1
	`
	row := r.psql.QueryRowContext(ctxWithTimeout, query, symbol)
...
func (r *InstrumentRepository) RemoveOneInstrument(ctx context.Context, symbol string) error {
	...
	result, err := r.psql.ExecContext(ctxWithTimeout, query, symbol)
	if err != nil {
...
func NewInstrumentRepository(
	...
	utils.ValidateTimeout("instrument repository", timeout)
